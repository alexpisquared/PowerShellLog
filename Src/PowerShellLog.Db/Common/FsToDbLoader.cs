...
namespace PowerShellLog.Db.Common
{
  ...
  {
    public async Task<string> LoadUpdateDbFromFs(A0DbContext db, string srcFileMode)
    {
...
;
    ...
    async Task<string> doOneFile(string pathfile, DateTime now, A0DbContext db)
    {
      var ttlNewCmds = 0;
      var ttlNewLogs = 0;
      var report = "";
    ...
    static string cleanCmd(string[] fsLines, int i)
    {
      return fsLines[i].Replace("     ", " ").Replace("    ", " ").Replace("   ", " ").Replace("  ", " ").Trim();
    }
  }
}
